...
//	if ((uState & UISTATE_READONLY) != 0)
//		clrText = pManager->GetThemeColor(UICOLOR_EDIT_TEXT_READONLY);
//	if ((uState & UISTATE_DISABLED) != 0)
//		clrText = pManager->GetThemeColor(UICOLOR_EDIT_TEXT_DISABLED);
//	::SetBkMode(hDC, TRANSPARENT);
...
//	::DestroyIcon(hIcon);
//	//::DrawFrameControl(hDC, &rcButton, DFC_BUTTON, DFCS_BUTTONRADIO | DFCS_CHECKED);  ????????????
//	// Paint text
...
//					if (pt.x == rc.left)
//						iLineIndent = iSize + (iSize / 8); else cyLine = MAX(iSize, cyLine);
//					pt.x += iSize + (iSize / 8);
...
void CDefaultRenderEngineUI::FillRect(HDC hDC, RECT rc, DWORD clrFill)
{
	if (clrFill <= 0x00FFFFFF)
		return;

	if (clrFill >= 0xFF000000)
	{
		::SetBkColor(hDC, RGB(GetBValue(clrFill), GetGValue(clrFill), GetRValue(clrFill)));
		::ExtTextOut(hDC, 0, 0, ETO_OPAQUE, &rc, NULL, 0, NULL);
	}
	else
	{
		...
		*pDest = clrFill;
		...
		RECT rcBmpPart = {0, 0, 1, 1};
		RECT rcCorners = {0};
		CRenderEngine::DrawImage(hDC, hBitmap, rc, rc, rcBmpPart, rcCorners, true, 255);
		::DeleteObject(hBitmap);
	}
}

void CDefaultRenderEngineUI::Rectangle(HDC hDC, RECT rc, DWORD dwFillColor, DWORD dwBorderColor, int nSize /*= 1*/, int nStyle /*= PS_SOLID*/)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);

	LOGPEN lg;
	lg.lopnColor = RGB(GetBValue(dwBorderColor), GetGValue(dwBorderColor), GetRValue(dwBorderColor));
	lg.lopnStyle = nStyle;
	lg.lopnWidth.x = nSize;
	HPEN hPen = CreatePenIndirect(&lg);
	HPEN hOldPen = (HPEN)::SelectObject(hDC, hPen);
	HBRUSH hBrush = dwFillColor <= 0x00FFFFFF ? (HBRUSH)::GetStockObject(HOLLOW_BRUSH) : CreateSolidBrush(dwFillColor);
...
void CDefaultRenderEngineUI::DrawRoundRect(HDC hDC, const RECT& rc, int nSize, int nWidth, int nHeight, DWORD dwPenColor)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
...
void CDefaultRenderEngineUI::DrawLine(HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor, int nStyle /*= PS_SOLID*/)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
...
void CDefaultRenderEngineUI::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
...
void CDefaultRenderEngineUI::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, int& nLinkRects, UINT uStyle)
{
	// 考虑到在xml编辑器中使用<>符号不方便，可以使用{}符号代替
	// 支持标签嵌套（如<l><b>text</b></l>），但是交叉嵌套是应该避免的（如<l><b>text</l></b>）
	// The string formatter supports a kind of "mini-html" that consists of various short tags:
...
//	//   Link:             <a x>text</a>        where x(optional) = link content, normal like app:notepad or http:www.xxx.com
	//   NewLine           <n>                  
	//   Paragraph:        <p x>		        where x = extra pixels indent in p
	//   Selected:         <s>text</s>
	//   Underline:        <u>text</u>
	//   X Indent:         <x i>                where i = hor indent in pixels
 	//   Y Indent:         <y i>                where i = ver indent in pixels 
	...
	if ((uStyle & DT_SINGLELINE) != 0 && (uStyle & DT_VCENTER) != 0 && (uStyle & DT_CALCRECT) == 0)
	{
		RECT rcText = { 0, 0, 9999, 100 };
		int nLinks = 0;
		DrawHtmlText(hDC, pManager, rcText, pstrText, dwTextColor, NULL, nLinks, uStyle | DT_CALCRECT);
		rc.top = rc.top + ((rc.bottom - rc.top) / 2) - ((rcText.bottom - rcText.top) / 2);
		rc.bottom = rc.top + (rcText.bottom - rcText.top);
	}
	...
	TEXTMETRIC tm = pManager->GetDefaultFontInfo()->tm;
	POINT pt = { rc.left, rc.top };
	int iLineIndent = 0;
	int iLinkIndex = 0;
	int cyLine = tm.tmHeight + tm.tmExternalLeading;
	...
	while ( *pstrText != '\0' ) 
	{
			...
			&& (pstrText[2] == ' ' || pstrText[2] == '>'))
		{
			...
			case 'a':  // Link
				{
					COLORREF cl =  bHoverLink ? pManager->GetDefaultLinkHoverFontColor() : pManager->GetDefaultLinkFontColor();
					::SetTextColor(hDC, RGB(GetBValue(cl), GetGValue(cl), GetRValue(cl)));
					TFontInfo* pFontInfo = pManager->GetDefaultFontInfo();
			...
			case 'b':  // Bold text
				{
					...
					if (!pFontInfo->bBold)
					{
						HFONT hFont = pManager->GetFont(pFontInfo->sFontName, pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);
						if (hFont == NULL)
							hFont = pManager->AddFont(pFontInfo->sFontName, pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);
						pFontInfo = pManager->GetFontInfo(hFont);
						tm = pFontInfo->tm;
						::SelectObject(hDC, pFontInfo->hFont);
						cyLine = MAX(cyLine, tm.tmHeight + tm.tmExternalLeading);
			...
			case 'f':  // Font
				{
					...
					tm = pManager->GetFontInfo(iFont)->tm;
					cyLine = MAX(cyLine, tm.tmHeight + tm.tmExternalLeading);
				}
				break;
			case 'h':  // Horizontal line
				{
					HPEN hPen = CreatePen(PS_SOLID, 1, RGB(GetBValue(pManager->GetDefaultDisabledColor()), GetGValue(pManager->GetDefaultDisabledColor()), GetRValue(pManager->GetDefaultDisabledColor())));
					HPEN hOldPen = (HPEN)::SelectObject(hDC, hPen);
					if (bDraw)
					{
						POINT ptTemp = { 0 };
			...
			case 'n':	// Newline
				{
					...
					cyLine = MAX(cyLine, tm.tmHeight + tm.tmExternalLeading) + 5;
					iLineIndent = 0;
					::SelectObject(hDC, pManager->GetDefaultFontInfo()->hFont);
					::SetTextColor(hDC, dwTextColor);
					tm = pManager->GetDefaultFontInfo()->tm;
				}
				break;
			case 'p':  // Paragraph
				{
					...
					while (*pstrText > '\0' && *pstrText <= ' ')
						pstrText = ::CharNext(pstrText);
					int cyLineExtra = (int)_tcstol(pstrText, const_cast<LPTSTR*>(&pstrText), 10);
					cyLine = MAX(cyLine, tm.tmHeight + tm.tmExternalLeading) + cyLineExtra;
					iLineIndent = 0;
					::SelectObject(hDC, pManager->GetDefaultFontInfo()->hFont);
					::SetTextColor(hDC, dwTextColor);
					tm = pManager->GetDefaultFontInfo()->tm;
...
void CDefaultRenderEngineUI::DoPaintFrame(HDC hDC, CPaintManagerUI* pManager, RECT rc, DWORD dwLight, DWORD dwDark, DWORD dwBackground /*= 0x00000000*/, UINT uStyle /*= 0*/)
{
	if (dwBackground != 0x00000000)
	{
		FillRect(hDC, rc, dwBackground);
	}
	if( (uStyle & UIFRAME_ROUND) != 0)
	{
		POINT ptTemp;
		HPEN hPenLight = CreatePen(PS_SOLID, 1, dwLight);
		HPEN hOldPen = (HPEN)::SelectObject(hDC, hPenLight);
		::MoveToEx(hDC, rc.left, rc.bottom - 2, &ptTemp);
		::LineTo(hDC, rc.left, rc.top + 1);
		::LineTo(hDC, rc.left + 1, rc.top);
		::LineTo(hDC, rc.right - 2, rc.top);
		HPEN hPenDark = CreatePen(PS_SOLID, 1, dwDark);
	...
	if ((uStyle & UIFRAME_FOCUS) != 0)
	{
		HPEN hPen = ::CreatePen(PS_DOT, 1, RGB(140, 140, 140));
		HPEN hOldPen = (HPEN) ::SelectObject(hDC, hPen);
		POINT ptTemp;
